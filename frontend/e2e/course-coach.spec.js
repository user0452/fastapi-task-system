...
test('资料、文件题库、诊断、Practice、Tutor Check 与 Student Model 闭环', async ({ page }, testInfo) => {
  ...
  const courseId = Number(page.url().match(/#\/materials\/(\d+)/)?.[1])
  ...
  await page.getByRole('button', { name: '开始快速定位', exact: true }).click()
  ...
  await diagnostic.getByRole('button', { name: '完成定位', exact: true }).click()
  await expect(diagnostic).toContainText('定位完成')
  await expect(page.locator('.evidence-result').last()).toContainText('诊断完成')
  ...
  await expect(page.locator('.tutor-response')).toContainText('学习助手')
  await expect(page.locator('.tutor-response')).toContainText('来自课程资料')
  await page.getByRole('button', { name: '检查理解' }).first().click()
  ...
  await checkForm.getByRole('button', { name: '提交回答', exact: true }).click()
  await expect(page.locator('.evidence-result').filter({ hasText: '掌握度' })).toContainText('刚刚完成')

  await page.goto(`/#/progress/${courseId}`)
  await expect(page.locator('.progress-view')).toBeVisible()
  await expect(page.locator('.objective-row')).toHaveCount(3)
  await expect(page.locator('.objective-detail')).toContainText('最近学习记录')
  await expect(page.locator('.objective-detail')).toContainText('把握度')

  await page.goto(`/#/materials/${courseId}`)
  await expect(page.locator('.sources-view')).toBeVisible()
