...
import { Bug, LogOut, Save, Settings2 } from 'lucide-vue-next'
...
async function savePreferences() {
  ...
  if (response.code === 200) {
    await courses.load()
    showToast({ type: 'success', message: '课程约束已更新，Learning Policy 会在下一次决策中使用' })
  } else {
...
<template>
  <div class="settings-page">
    <header class="page-heading">
      ...
      <h1>设置</h1>
      <p>这里只保留影响课程学习决策的约束；掌握度、错误模式和 Evidence 都在 Progress 中。</p>
    </header>

    <div class="settings-layout">
      <section class="settings-section account-section">
        <div class="section-heading"><Settings2 :size="19" /><div><h2>账号</h2><p>{{ auth.username }} · Student Model 按课程隔离。</p></div></div>
      ...
      <section class="settings-section">
        <div class="section-heading"><Settings2 :size="19" /><div><h2>当前课程约束</h2><p>{{ courses.current?.name || '尚未选择课程' }} · 考试日期和学习时长只作为 Policy constraint。</p></div></div>
        <form v-if="courses.current" class="preference-form" @submit.prevent="savePreferences">
          ...
          <button class="primary-button" type="submit" :disabled="saving"><Save :size="15" /> {{ saving ? '正在保存' : '保存约束' }}</button>
        </form>
        <button v-else class="text-button" type="button" @click="router.push('/today')">先创建一门课程</button>
      </section>

      <section class="settings-section developer-section">
        <div class="section-heading"><Bug :size="19" /><div><h2>开发模式</h2><p>只影响日志入口，不会改变 Learning Policy 或 Student Model。</p></div></div>
        <label class="toggle-row"><div><strong>显示操作日志</strong><span>需要排查 ingestion、retrieval 或 grading 时临时打开。</span></div><input v-model="developerMode" type="checkbox" @change="toggleDeveloperMode" /><i></i></label>
        <button v-if="developerMode" class="secondary-button log-button" type="button" @click="router.push('/logs')">查看操作日志</button>
      </section>
    </div>
  </div>
</template>
...
.primary-button, .secondary-button, .text-button { min-height: 42px; display: inline-flex; align-items: center; justify-content: center; gap: 7px; padding: 0 15px; border-radius: 13px; font-size: 13px; font-weight: 600; }.primary-button { color: var(--text-inverse); background: var(--gradient-brand); box-shadow: var(--shadow-brand); }.secondary-button { color: var(--accent); border: 1px solid var(--border-accent); background: var(--surface-primary); }.text-button { margin: 0 18px 16px; color: var(--accent); padding: 0; }.log-button { margin: 0 18px 16px; }
.toggle-row { position: relative; display: flex; align-items: center; gap: 14px; padding: 19px 22px; cursor: pointer; }.toggle-row > div { flex: 1; display: grid; gap: 4px; }.toggle-row strong { font-size: 14px; }.toggle-row span { color: var(--text-secondary); font-size: 12px; }.toggle-row input { position: absolute; opacity: 0; }.toggle-row i { position: relative; width: 44px; height: 26px; border-radius: var(--radius-round); background: #c7c8cd; transition: background var(--duration-fast) var(--ease-standard); }.toggle-row i::after { content: ''; position: absolute; left: 3px; top: 3px; width: 20px; height: 20px; border-radius: 50%; background: var(--surface-primary); transition: transform var(--duration-fast) var(--ease-standard); }.toggle-row input:checked + i { background: var(--accent); }.toggle-row input:checked + i::after { transform: translateX(18px); }
