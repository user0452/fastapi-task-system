...
def _require_course_agent(cursor, user_id: int, course_id: int) -> tuple[dict, dict]:
    course = course_repository.get_course(cursor, course_id, user_id)
    if course is None or course.get("status") == "archived":
        raise AppError("课程不存在或无访问权限", 404, "COURSE_NOT_FOUND")
...
def list_course_agent_memories(user_id: int, course_id: int) -> dict:
    with get_cursor() as cursor:
        _, agent = _require_course_agent(cursor, user_id, course_id)
    ...
    return {
        "items": [_public_memory(memory) for memory in memories],
        "total": len(memories),
    }
...
def update_course_agent_memory(
    ...
    with get_cursor() as cursor:
        _require_course_agent(cursor, user_id, course_id)
