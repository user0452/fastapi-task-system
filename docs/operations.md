...
2. 执行 `.venv\Scripts\python.exe -m app.core.migrations`。
...
2. 检查 `schema_migrations` 是否已执行到最新版本。
...
完整验收包括迁移、编译、Ruff、Mypy、关键路径覆盖率、ESLint、Vitest、生产构建和 Playwright。CI 使用相同的静态检查和测试门槛。
